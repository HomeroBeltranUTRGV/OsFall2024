...
impl MLFQ {
    ...
    pub fn add_process(&mut self, mut process: Process) {
        // If the priority is greater than the max level, cap it to the highest level
        if process.priority >= self.num_levels {
            println!("Warning: Adjusting invalid priority for process {}.", process.id);
    ...
    pub fn execute_process(&mut self, queue_index: usize) {
        ...
        if queue_index >= self.num_levels {
            println!("Error: Invalid queue index {}", queue_index);
            return;
        }
        if let Some(mut process) = self.queues[queue_index].pop() {
            let time_quantum = self.time_quanta[queue_index];
            let time_to_execute = std::cmp::min(time_quantum, process.remaining_time);

            // Execute the process for its time quantum or until completion
            process.remaining_time -= time_to_execute;
            process.total_executed_time += time_to_execute;
            self.current_time += time_to_execute;  // Update global system time

            if process.remaining_time > 0 {
                // Process didn't finish, demote it to a lower-priority queue (if possible)
                if process.priority < self.num_levels - 1 {
                    process.priority += 1;  // Move to the next lower priority level
                }
                self.add_process(process);
            } else {
                println!(
                    "Process {} completed execution at time {}.",
                    process.id, self.current_time
                );
            }
        } else {
            println!("No process available in queue {}", queue_index);
        }
    }
    ...
    pub fn priority_boost(&mut self) {
        ...
        // Temporary vector to collect all processes
        let mut all_processes: Vec<Process> = Vec::new();

        // Collect processes from all queues
        for priority_level in 0..self.num_levels {
            while let Some(process) = self.queues[priority_level].pop() {
                all_processes.push(process);
            }
        }

        // Reset priority of all processes to 0 and move them to the highest priority queue
        for mut process in all_processes {
            process.priority = 0;  // Reset priority to the highest level
            self.queues[0].push(process);
        }

        println!("Priority boost: All processes moved to the highest priority queue.");
    }
